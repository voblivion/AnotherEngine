...
namespace aoe
{
	namespace type
	{
		namespace detail
		{
			...
			template <typename VoidMaybeConst, typename Type
		...
		template <typename VoidMaybeConst, template <typename> typename Func
			, typename... Args>
		class Applicator
		{
			// Aliases
		...
		public:
			...
			void registerType(Func<Type> a_functor = {})
			{
				...
				m_typeApplicators.emplace(typeid(Type)
					, sta::allocatePolymorphicWith<TypeApplicator<Type>>(
						getResource(), a_functor));
			}

			template <typename BaseType>
			void apply(BaseType& a_object, Args&&... a_args) const
			{
				ignorableAssert(isRegistered(typeid(a_object)));
		...
		private:
			...
			std::pmr::unordered_map<std::type_index
				, sta::PolymorphicPtr<ATypeApplicator>> m_typeApplicators;
		};
	}
}
