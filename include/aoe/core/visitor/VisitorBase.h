...
#include <aoe/core/visitor/Traits.h>
...
namespace aoe
{
	namespace vis
	{
		...
		public:
			// Constructors
			explicit VisitorBase(sta::TypeRegistry const& a_typeRegistry
		...
		private:
			// Attributes
			sta::TypeRegistry const& m_typeRegistry;
		...
		public:
			// Constructor
			explicit OutputVisitorBase(sta::TypeRegistry const& a_typeRegistry
				, sta::TypeFactory const& a_typeFactory
		...
		public:
			// Constructor
			explicit InputVisitorBase(sta::TypeRegistry const& a_typeRegistry
				, sta::TypeFactory const& a_typeFactory
				, sta::Allocator<std::byte> a_allocator = {})
				: VisitorBase{ a_typeRegistry, a_typeFactory }
				, m_applicator{ a_allocator }
		...
		protected:
			// Methods
			template <typename ValueType
				, enforce((hasMemberAccept<VisitorType, ValueType>))>
			void processVisit(ValueType&& a_value)
			{
				std::forward<ValueType>(a_value).accept(static_cast<VisitorType&>(*this));
			}
			...
			template <typename ValueType
				, enforce((hasNonMemberAccept<VisitorType, ValueType>))>
				void processVisit(ValueType&& a_value)
			{
				accept(static_cast<VisitorType&>(*this), std::forward<ValueType>(a_value));
			}
