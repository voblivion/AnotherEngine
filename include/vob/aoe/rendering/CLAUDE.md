...
OpenGL 4.6 forward+ renderer: light clustering, CSM sun shadows + spot shadows, depth pre-pass, opaque g-buffer (normal/surface/geometric-normal/depth), SSAO, SSR, opaque composition, translucent, skybox, post-process, debug geometry, ImGui.
...
- `systems/` — `RenderSceneSystem` (main draw), `DebugCameraDirectorSystem`, `DebugRenderLightsSystem`, `ReleaseUnusedGpuResourcesSystem` (drains registries), `PrepareImGuiFrameSystem`/`RenderImGuiFrameSystem`, `SwapBuffersSystem`.
