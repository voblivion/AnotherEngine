...
- `resource/` (`vob::aoe::res`) — old resource holder/handle (`AHolder`, `DefaultHolder`, `Handle`). Marked for removal in the migration map; superseded in practice by `rendering/resources/Handle.h`/`WeakHandle.h`.
