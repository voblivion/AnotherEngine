...
namespace vob::aoegl
{
	namespace
	{
		constexpr auto k_ktx2Identifier =
			std::array<uint8_t, 12>{0xAB, 0x4B, 0x54, 0x58, 0x20, 0x32, 0x30, 0xBB, 0x0D, 0x0A, 0x1A, 0x0A};
		...
		bool isKtx2(std::istream& a_stream)
		{
			...
			return readCount == static_cast<std::streamsize>(identifier.size()) && identifier == k_ktx2Identifier;
		}
		...
		ImageData loadKtx2Image(std::istream& a_stream)
		{
			...
			auto const levelIndexByteSize = static_cast<std::streamsize>(levelCount * sizeof(LevelIndexEntry));
			...
			if (!VOB_AOE_CHECK_LOG(a_stream.gcount() == levelIndexByteSize, "Truncated ktx2 level index."))
			{
				return ImageData{};
			}

			auto image = ImageData{.format = formatIt->format, .colorSpace = formatIt->colorSpace};

			auto dataOffset = size_t{0};
			for (auto levelIt = int32_t{0}; levelIt < levelCount; ++levelIt)
			{
				auto const levelSize =
					glm::uvec2{std::max(header.pixelWidth >> levelIt, 1u), std::max(header.pixelHeight >> levelIt, 1u)};
			...
			for (auto levelIt = int32_t{0}; levelIt < levelCount; ++levelIt)
			{
				...
				if (!VOB_AOE_CHECK_LOG(a_stream.gcount() == levelByteSize, "Truncated ktx2 level {}.", levelIt))
				{
					return ImageData{};
				}
			}
		...
		ImageData loadStbImage(std::istream& a_stream)
		{
			...
			uint8_t* data = stbi_load_from_callbacks(
				&callbacks, &a_stream, &width, &height, &channelCount, 0 /* required channel count */);
