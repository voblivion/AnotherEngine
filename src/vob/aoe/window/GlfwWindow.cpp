...
namespace vob::aoewi
{
		...
		GLFWwindow* const a_share)
	{
		...
		{
			m_knownMonitors.push_back(KnownMonitor{ monitors[monitorIndex], m_monitors.getId(monitorIndex) });
		}
	...
	IMonitors const& GlfwWindow::getMonitors() const
	{
		return m_monitors;
	}
	...
	DisplayMode GlfwWindow::getActiveDisplayMode() const
	{
		auto displayMode = DisplayMode{};
		auto monitorIndex = std::optional<int32_t>{};
		auto* const windowMonitor = glfwGetWindowMonitor(m_nativeHandle);
		if (windowMonitor != nullptr)
		{
			displayMode.mode = WindowMode::FullScreen;
			auto monitorCount = 0;
			auto** const monitors = glfwGetMonitors(&monitorCount);
			for (auto i = 0; i < monitorCount; ++i)
			{
				if (monitors[i] == windowMonitor)
				{
					monitorIndex = i;
					break;
				}
		...
		{
			displayMode.mode = glfwGetWindowAttrib(m_nativeHandle, GLFW_DECORATED) == GLFW_TRUE
				? WindowMode::Windowed
				: WindowMode::Borderless;
			monitorIndex = m_monitors.getIndexAt(getPosition() + getSize() / 2);
		}

		displayMode.size = getSize();
		if (!monitorIndex.has_value())
		{
			displayMode.position = getPosition();
			return displayMode;
		}

		auto monitorCount = 0;
		auto** const monitors = glfwGetMonitors(&monitorCount);
		auto const knownMonitor = std::ranges::find(m_knownMonitors, monitors[*monitorIndex], &KnownMonitor::handle);
		if (knownMonitor != m_knownMonitors.end())
		{
			displayMode.monitorId = knownMonitor->id;
		}
		displayMode.position = getPosition() - m_monitors.getPosition(*monitorIndex);
		return displayMode;
	}

	void GlfwWindow::requestDisplayModeChange(DisplayMode const& a_displayMode)
	{
		auto const isSizeValid
			= a_displayMode.mode == WindowMode::Borderless || (a_displayMode.size.x > 0 && a_displayMode.size.y > 0);
		if (!VOB_AOE_CHECK_LOG(isSizeValid, "Ignoring display mode change with an invalid size."))
		{
			return;
		}

		m_desiredDisplayMode = a_displayMode;
		m_hasRequestedDisplayModeChange = true;
	}

	void GlfwWindow::pollEvents()
	{
		m_events.clear();
		applyRequestedDisplayMode();
		glfwPollEvents();
		reconcileMonitorChanges();

		if (glfwGetWindowAttrib(m_nativeHandle, GLFW_ICONIFIED) == GLFW_TRUE)
		{
			return;
		}

		auto displayMode = getActiveDisplayMode();
		if (displayMode != m_lastDisplayMode)
		{
			m_lastDisplayMode = std::move(displayMode);
			pushEvent(DisplayModeChangedEvent{});
		}
	}
	...
	void GlfwWindow::setVSyncEnabled(bool const a_enabled)
	{
	...
	bool GlfwWindow::isGamepadPresent(int32_t a_gamepadIndex) const
	{
		return glfwJoystickPresent(GLFW_JOYSTICK_1 + a_gamepadIndex) != 0;
	}

	bool GlfwWindow::isGamepadButtonPressed(int32_t a_gamepadIndex, aoein::Gamepad::Button a_button) const
	{
		auto result = false;
	...
	float GlfwWindow::getGamepadAxisValue(int32_t a_gamepadIndex, aoein::Gamepad::Axis a_axis) const
	{
		GLFWgamepadstate gamepadState;
		if (!glfwGetGamepadState(GLFW_JOYSTICK_1 + a_gamepadIndex, &gamepadState))
		{
		...
		return gamepadState.axes[static_cast<int32_t>(a_axis)];
	}
	...
	void GlfwWindow::keyEventCallback(
		GLFWwindow* a_nativeHandle, GLint a_keyCode, GLint a_scanCode, GLint a_action, GLint a_modifierMask)
	{
		auto const window = static_cast<GlfwWindow*>(glfwGetWindowUserPointer(a_nativeHandle));

		static constexpr std::array<aoein::Keyboard::Key, GLFW_KEY_MENU + 1 - GLFW_KEY_SPACE> k_keyCodeToKey{
			...
			aoein::Keyboard::Key::RSystem, // 347 GLFW_KEY_RIGHT_SUPER
			aoein::Keyboard::Key::Menu // 348 GLFW_KEY_MENU
		};
		if (a_keyCode - GLFW_KEY_SPACE < 0 || a_keyCode - GLFW_KEY_SPACE >= mistd::isize(k_keyCodeToKey))
		{
			return;
		}
	...
	void GlfwWindow::textEventCallback(GLFWwindow* a_nativeHandle, GLuint a_unicode)
	{
		auto const window = static_cast<GlfwWindow*>(glfwGetWindowUserPointer(a_nativeHandle));
		window->pushEvent(TextEvent{ a_unicode });
	}

	void GlfwWindow::mouseMoveEventCallback(GLFWwindow* a_nativeHandle, GLdouble a_x, GLdouble a_y)
	{
		auto const window = static_cast<GlfwWindow*>(glfwGetWindowUserPointer(a_nativeHandle));
		window->pushEvent(MouseMoveEvent{ { static_cast<float>(a_x), static_cast<float>(a_y) } });
	}

	void GlfwWindow::mouseEnterEventCallback(GLFWwindow* a_nativeHandle, GLint a_entered)
	{
		auto const window = static_cast<GlfwWindow*>(glfwGetWindowUserPointer(a_nativeHandle));
		window->pushEvent(MouseHoverEvent{ a_entered == GLFW_TRUE });
	}

	void GlfwWindow::mouseButtonEventCallback(
		GLFWwindow* a_nativeHandle, GLint a_button, GLint a_action, GLint a_modifierMask)
	{
		auto const window = static_cast<GlfwWindow*>(glfwGetWindowUserPointer(a_nativeHandle));
	...
	void GlfwWindow::mouseScrollEventCallback(GLFWwindow* a_nativeHandle, GLdouble a_x, GLdouble a_y)
	{
		auto const window = static_cast<GlfwWindow*>(glfwGetWindowUserPointer(a_nativeHandle));
		window->pushEvent(MouseScrollEvent{ { static_cast<float>(a_x), static_cast<float>(a_y) } });
	}

	void GlfwWindow::applyRequestedDisplayMode()
	{
		if (!m_hasRequestedDisplayModeChange)
		{
			return;
		}
		m_hasRequestedDisplayModeChange = false;

		auto* const monitor = getMonitorHandle(m_desiredDisplayMode.monitorId);
		if (monitor == nullptr)
		{
			return;
		}
		...
		glfwSetWindowAttrib(
			m_nativeHandle, GLFW_DECORATED, m_desiredDisplayMode.mode == WindowMode::Windowed ? GLFW_TRUE : GLFW_FALSE);

		switch (m_desiredDisplayMode.mode)
		{
		case WindowMode::Windowed:
			glfwSetWindowMonitor(
				m_nativeHandle,
				nullptr,
				monitorPosition.x + m_desiredDisplayMode.position.x,
				monitorPosition.y + m_desiredDisplayMode.position.y,
				m_desiredDisplayMode.size.x,
				m_desiredDisplayMode.size.y,
				GLFW_DONT_CARE);
			break;
		...
		if (m_desiredDisplayMode.mode != WindowMode::FullScreen)
		{
			auto const size = m_desiredDisplayMode.mode == WindowMode::Borderless
				? glm::ivec2{ videoMode->width, videoMode->height }
				: m_desiredDisplayMode.size;
			glfwSetWindowSize(m_nativeHandle, size.x, size.y);
		}
	}

	void GlfwWindow::reconcileMonitorChanges()
	{
		auto monitorCount = 0;
		auto** const monitors = glfwGetMonitors(&monitorCount);
		auto const lostMonitorCount = std::erase_if(
			m_knownMonitors,
			[&](KnownMonitor const& a_knownMonitor)
			{
				return std::find(monitors, monitors + monitorCount, a_knownMonitor.handle) == monitors + monitorCount;
			});
		...
		for (auto monitorIndex = 0; monitorIndex < monitorCount; ++monitorIndex)
		{
			...
			if (std::ranges::find(m_knownMonitors, monitor, &KnownMonitor::handle) == m_knownMonitors.end())
			{
				m_knownMonitors.push_back(KnownMonitor{ monitor, m_monitors.getId(monitorIndex) });
				newMonitors.push_back(monitor);
			}
		}

		if (lostMonitorCount > 0)
		{
			m_desiredDisplayMode.monitorId = getActiveDisplayMode().monitorId;
			if (m_desiredDisplayMode.mode != WindowMode::Windowed)
			{
				auto const monitorIndex = m_monitors.getIndex(m_desiredDisplayMode.monitorId);
				if (monitorIndex.has_value())
				{
					m_desiredDisplayMode.size = m_monitors.getResolution(*monitorIndex);
				}
			}
			m_hasRequestedDisplayModeChange = true;
		}
		...
		// Rendering a single frame on any reconnected monitor then moving away releases them to the OS.
		// This issue only occurs on monitors we presented to, but we touch any reconnected monitor for simplicity.
		auto* const savedMonitor = glfwGetWindowMonitor(m_nativeHandle);
		auto const savedDisplayMode = getActiveDisplayMode();
		...
		for (auto* const monitor : newMonitors)
		{
			...
			glfwGetMonitorPos(monitor, &monitorPosition.x, &monitorPosition.y);
			glfwSetWindowMonitor(
				m_nativeHandle, nullptr, monitorPosition.x, monitorPosition.y, videoMode->width, videoMode->height, 0);
			glfwSwapBuffers(m_nativeHandle);
		}
		...
		{
			glfwSetWindowMonitor(
				m_nativeHandle, savedMonitor, 0, 0, savedDisplayMode.size.x, savedDisplayMode.size.y, GLFW_DONT_CARE);
		}
		else
		{
			auto const monitorIndex = m_monitors.getIndex(savedDisplayMode.monitorId);
			auto const monitorPosition
				= monitorIndex.has_value() ? m_monitors.getPosition(*monitorIndex) : glm::ivec2{};
			auto const restorePosition = monitorPosition + savedDisplayMode.position;
			glfwSetWindowPos(m_nativeHandle, restorePosition.x, restorePosition.y);
			glfwSetWindowSize(m_nativeHandle, savedDisplayMode.size.x, savedDisplayMode.size.y);
		}
	}

	GLFWmonitor* GlfwWindow::getMonitorHandle(std::string_view const a_monitorId) const
	{
		auto const monitorIndex = m_monitors.getIndex(a_monitorId);
		auto monitorCount = 0;
		auto** const monitors = glfwGetMonitors(&monitorCount);
		if (!monitorIndex.has_value() || monitors == nullptr || *monitorIndex >= monitorCount)
		{
			return nullptr;
		}
	...
	void GlfwWindow::pushEvent(WindowEvent a_event)
	{
		m_events.emplace_back(std::move(a_event));
	}
}
