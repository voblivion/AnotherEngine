...
namespace vob::aoeph
{
	...
	// using broadphase_result = std::tuple<triangle, glm::vec3, glm::quat, material>;
	...
	void physics_debug_system::update() const
	{
		...
		if (physicsDebugContext.is_dynamic_shape_debug_enabled)
		{
			for (auto [entity, position, rotation, linearVelocity, angularVelocityLocal, carCollider] : m_carColliderEntities.get().each())
			{
				...
				for (auto const& chassisPart : carCollider.chassisParts)
				{
					...
					m_debugMeshContext->add_ellipsoid(ellipsoidTransform, chassisPart.radiuses, aoegl::_k_white);

				}

				for (auto const& wheel : carCollider.wheels)
				{
					auto const wheelRotation = rotation * wheel.rotation * glm::angleAxis(wheel.turn, glm::vec3{ 0.0f, 1.0f, 0.0f });
					auto const wheelDown = wheelRotation * glm::vec3{ 0.0f, -1.0f, 0.0f };
					auto const wheelPosition = position + rotationMatrix * wheel.attachPosition + wheel.suspensionLength * wheelDown;

					auto const ellipsoidTransform = aoest::combine(wheelPosition, wheelRotation);
					auto const wheelColor = wheel.suspensionLength == 0 ? aoegl::_k_forest : (wheel.isTireGrounded ? aoegl::_k_green : aoegl::_k_red);
					m_debugMeshContext->add_ellipsoid(ellipsoidTransform, wheel.radiuses, wheelColor);
				}
			}
		}
		...
		if (physicsDebugContext.is_dynamic_contact_debug_enabled)
		{
			for (auto [entity, position, rotation, linearVelocity, angularVelocityLocal, carCollider] : m_carColliderEntities.get().each())
			{
				auto barycenter = position + rotation * carCollider.barycenter;
				m_debugMeshContext->add_sphere(barycenter, 0.1f, aoegl::_k_azure);

				auto const stepCount = 4 * static_cast<float>(m_physicsContext.get().updateStepCount);

				for (auto [pos, dir] : carCollider.debugUpdateForces)
				{
					m_debugMeshContext->add_line(pos, pos + dir, aoegl::_k_red);
				}

				static float kMaxPosDist = 0.1f;
				static float kMaxDirDist = 0.1f;
				{
					std::vector<std::vector<std::pair<glm::vec3, glm::vec3>>> allMergedForces;

					for (auto [pos, dir] : carCollider.debugRk4StepForceOver100s)
					{
						int32_t foundIndex = -1;
						int32_t index = 0;
						for (auto& mergedForces : allMergedForces)
						{
							++index;
							if (glm::length(mergedForces[0].first - pos) > kMaxPosDist)
							{
								continue;
							}

							if (1.0f - glm::dot(mergedForces[0].second, dir) / (glm::length(mergedForces[0].second) * glm::length(dir)) > kMaxDirDist)
							{
								continue;
							}
						...
						if (foundIndex != -1)
						{
							allMergedForces[foundIndex].emplace_back(pos, dir);
						}
						else
						{
							std::vector<std::pair<glm::vec3, glm::vec3>> mergedForces;
							mergedForces.emplace_back(pos, dir);
							allMergedForces.push_back(mergedForces);
						}
					}
					...
					for (auto& mergedForces : allMergedForces)
					{
						...
						for (auto [pos, dir] : mergedForces)
						{
							totalPos += pos;
							totalDir += dir;
						}

						auto pos = totalPos / static_cast<float>(mergedForces.size());

						m_debugMeshContext->add_line(pos, pos + totalDir, aoegl::_k_orange);
					}
				}
				...
				for (auto const& wheel : carCollider.wheels)
				{
					...
					for (auto [pos, dir] : wheel.debugRk4StepForceOver100s)
					{
						int32_t foundIndex = -1;
						int32_t index = 0;
						for (auto& mergedForces : allMergedForces)
					...
					for (auto& mergedForces : allMergedForces)
					{
						...
						m_debugMeshContext->add_line(pos, pos + totalDir, aoegl::_k_yellow);
					}
				}
			}
		}

		if (physicsDebugContext.is_aabb_debug_enabled)
		{
			for (auto [entity, position, rotation, linearVelocity, angularVelocityLocal, carCollider] : m_carColliderEntities.get().each())
			{
				auto const carTransform = aoest::combine4x3(position, rotation);

				auto const searchBounds = compute_car_broadphase_bounds(carTransform, carCollider);

				//m_debugMeshContext->add_aabb(searchBounds.min, searchBounds.max, aoegl::_k_azure);
				m_debugMeshContext->add_obb(carTransform, carCollider.boundsLocal.min, carCollider.boundsLocal.max, aoegl::_k_azure);
			}
		}
		...
		ImGui::Checkbox("Draw Car Bounds", &physicsDebugContext.is_aabb_debug_enabled);
		ImGui::End();

		car_collider* mainCarCollider = nullptr;
		int32_t mainCarBroadphaseSize = 0;
		glm::vec3 mainCarPosition;
		glm::vec3 mainCarRotation;
		for (auto [entity, position, rotation, linearVelocity, angularVelocityLocal, carCollider] : m_carColliderEntities.get().each())
		{
			mainCarCollider = &carCollider;
			mainCarBroadphaseSize = carCollider.broadphaseSize;
			mainCarPosition = position;
			mainCarRotation = glm::eulerAngles(rotation);
		}
		if (mainCarCollider != nullptr)
		{
			static float chassisEllasticityOver100 = 80'000'0.0f;
			static float chassisRestitution = 0.01f;
			static float suspensionRestLength = 0.3f;
			static float suspensionEllasticityOver100 = 216.0f;
			static float suspensionDamperOver100 = 216.0f;
			static float tireEllasticityOver100 = 20'000'0.0f;
			static float tireRestitution = 0.01f;
			static float carMass = 1000.0f;
			static float wheelMass = 10.0f;
			static float compressedEllasticityOver100 = 20'000'0.f;
			static float compressedRestitution = 0.1f;

			glm::vec2 frontSuspensions{ mainCarCollider->wheels[0].suspensionLength, mainCarCollider->wheels[1].suspensionLength };
			glm::vec2 rearSuspensions{ mainCarCollider->wheels[2].suspensionLength, mainCarCollider->wheels[3].suspensionLength };

			ImGui::Begin("Car Collider");
			ImGui::InputFloat("Mass", &carMass);
			ImGui::InputFloat("Wheel Mass", &wheelMass);
			ImGui::InputFloat("Chassis Ellasticity / 100", &chassisEllasticityOver100);
			ImGui::InputFloat("Chassis Restitution", &chassisRestitution);
			ImGui::InputFloat("Suspension Rest Length", &suspensionRestLength);
			ImGui::InputFloat("Suspension Ellasticity / 100", &suspensionEllasticityOver100);
			ImGui::InputFloat("Suspension Damper / 100", &suspensionDamperOver100);
			ImGui::InputFloat("Tire Ellasticity / 100", &tireEllasticityOver100);
			ImGui::InputFloat("Tire Restitution", &tireRestitution);
			ImGui::InputFloat("Compressed Ellasticity / 100", &compressedEllasticityOver100);
			ImGui::InputFloat("Compressed Restitution", &compressedRestitution);
			ImGui::InputInt("Step Count", &m_physicsContext.get().updateStepCount);
			ImGui::BeginDisabled();
			ImGui::InputInt("Broadphase Size", &mainCarBroadphaseSize);
			ImGui::InputInt3("Intersection Test Count", &mainCarCollider->testCounts.x);
			ImGui::InputFloat3("Position", &mainCarPosition.x);
			ImGui::InputFloat3("Rotation", &mainCarRotation.x);
			ImGui::InputFloat2("Front Suspensions", &frontSuspensions.x);
			ImGui::InputFloat2("Rear Suspensions", &rearSuspensions.x);
			ImGui::EndDisabled();
			ImGui::End();

			mainCarCollider->mass = carMass;
			for (auto& chassisPart : mainCarCollider->chassisParts)
			{
				chassisPart.material.ellasticityOver100 = chassisEllasticityOver100;
				chassisPart.material.restitution = chassisRestitution;
			}

			for (auto& wheel : mainCarCollider->wheels)
			{
				wheel.rimMaterial.ellasticityOver100 = chassisEllasticityOver100;
				wheel.rimMaterial.restitution = chassisRestitution;
				wheel.suspensionRestLength = suspensionRestLength;
				wheel.suspensionEllasticityOver100 = suspensionEllasticityOver100;
				wheel.suspensionDamperOver100 = suspensionDamperOver100;
				wheel.tireMaterial.ellasticityOver100 = tireEllasticityOver100;
				wheel.tireMaterial.restitution = tireRestitution;
				wheel.compressedEllasticityOver100 = compressedEllasticityOver100;
				wheel.compressedRestitution = compressedRestitution;
